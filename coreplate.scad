...
module side_holes()
{
    for (i = [18 : 25.5 : 230]){
        ...
        translate([4.9+b_w-14,i,3.5]) rotate([0,90,0]) cylinder(r=6.8/2,h=2.5,$fn=6);
        ...
        translate([i,4.9+b_d-14,3.5]) rotate([-90,0,0]) cylinder(r=6.8/2,h=2.5,$fn=6);  
        }
}
...
module top_holes()
{
   for (i = [18+12.5 : 25.5 : 230]){     
       ...
       translate([3.5,i,-0.1]) rotate([0,0,0]) cylinder(r=6.8/2,h=2,$fn=6);
       translate([3.5+b_w-7.25,i,-1]) rotate([0,0,0]) cylinder(r=3.4/2,h=16,$fn=128);
       translate([3.5+b_w-7.25,i,-0.1]) rotate([0,0,45])  cylinder(r=6.8/2,h=2,$fn=6);
        
       translate([i,3.5,-1]) rotate([0,0,0]) cylinder(r=3.4/2,h=16,$fn=128);
       translate([i,3.5,-0.1]) rotate([0,0,0]) cylinder(r=6.8/2,h=2,$fn=6);
       translate([i,239.34+.5,-1]) rotate([0,0,0]) cylinder(r=3.4/2,h=16,$fn=128);
       translate([i,239.34+.5,-0.1]) rotate([0,0,0]) cylinder(r=6.8/2,h=2,$fn=6);
...
for (h = b_holes)
	{
        ...
        translate([h[0],h[1],1.5])
			cylinder( r=6.8/2, h = 5, center=true, $fn=6 );    
	}
